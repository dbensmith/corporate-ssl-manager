# Corporate SSL/TLS Certificate Manager for Docker
# Properly configures Docker daemon and containers to use corporate certificates globally
# Fixes all identified issues: PowerShell parsing, Docker daemon readiness, line endings

[CmdletBinding()]
param(
    [string[]]$SearchPatterns = @('CA', 'Netskope', 'Palo Alto', 'Zscaler'),
    ...
    [string]$EnterpriseServerCAPattern = '*My Enterprise Server CA*',
    [string]$EnterpriseServerCAFilename = 'my-enterprise-server.crt',
...
function Install-DockerSSLConfiguration {
    ...
    try {
        ...
        # Find and install enterprise server CA
        $serverCA = $ExportedCertificates | Where-Object { $_.Subject -like $EnterpriseServerCAPattern } | Select-Object -First 1
        if ($serverCA) {
            $serverCAPath = wsl wslpath "'$($serverCA.Filepath)'"
            wsl bash -c "sudo cp '$serverCAPath' /usr/local/share/ca-certificates/$EnterpriseServerCAFilename" 2>$null
            wsl bash -c 'sudo update-ca-certificates' 2>$null
            Write-ColorLog "Installed Enterprise Server CA system-wide: $EnterpriseServerCAFilename" -Level 'SUCCESS' -Color Green
        }
...
Write-ColorLog "Enterprise Server CA Pattern: $EnterpriseServerCAPattern" -Level 'INFO'
Write-ColorLog "Enterprise Server CA Filename: $EnterpriseServerCAFilename" -Level 'INFO'
Write-ColorLog "Docker Registries: $($DockerRegistries -join ', ')" -Level 'INFO'
