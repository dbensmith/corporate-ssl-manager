...
async function testDomains(domains, options = {}) {
  ...
  for (const domain of domains) {
    ...
    if (result.success) {
      console.log(
        `  OK ${result.url} - HTTP ${result.statusCode} (${result.duration}ms)`,
      );
    } else {
      console.log(
        `  FAIL ${result.url} - ${result.error} (${result.duration}ms)`,
      );
    }
  }
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case "--help":
        ...
        console.log("Options:");
        console.log(
          "  --cert <path>        Test with specific certificate file",
        );
        console.log(
          "  --domains <list>     Comma-separated list of domains to test",
        );
        console.log(
          "  --skip-insecure      Skip insecure (reject unauthorized = false) test",
        );
  ...
  if (!skipInsecure) {
    ...
    try {
      const insecureResults = await testDomains(domains, {
  ...
  if (currentSuccess === totalDomains) {
    console.log(
      "All domains are working correctly with current configuration.",
    );
  } else if (currentSuccess === 0) {
    console.log("No domains are working. Consider:");
    console.log(
      "   - Setting NODE_EXTRA_CA_CERTS to your corporate certificate bundle",
    );
    console.log("   - Checking network connectivity");
    console.log("   - Verifying corporate proxy/firewall settings");
  } else {
    console.log(
      `Partial connectivity (${currentSuccess}/${totalDomains} domains working).`,
    );
    console.log("   - Some domains may require additional certificates");
    console.log(
      "   - Check if specific domains are blocked by corporate policy",
    );
  }
}
